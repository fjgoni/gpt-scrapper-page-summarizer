...
url = "https://viainfo.com.ar/"  # replace with the real URL
scraper = WebScraper(url)
# text = scraper.get_contents()

html = scraper.scrape_full_page_html()
openai_client = OpenAIClient()
output = GPTAgent(client=openai_client, system_prompt=html_extractor_prompt).run(html)
summarized_page = GPTAgent(client=openai_client, system_prompt=summarizer_page_prompt).run(output)
